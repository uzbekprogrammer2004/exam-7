...
import categoryService from "../../../service/category2"; // Adjust the path as per your file structure
import productService from "../../../service/products"; // Adjust the path as per your file structure
...
const validationSchema = Yup.object().shape({
  ...
  size: Yup.array()
    .of(
      Yup.string()
        .matches(/^(M|CM|D|C|XC|L|XL|X|V|I)$/, {
          message: "Size must be Roman numerals (M, CM, D, C, XC, L, XL, X, V, I)",
...
const AddProductModal = ({ open, handleClose }) => {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      const response = await productService.add(values);
      if (response.status === 201) {
        window.location.reload();
      } else {
        console.log("Failed to save product:", response);
      }
    } catch (error) {
